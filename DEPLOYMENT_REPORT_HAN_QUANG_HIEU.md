...
3. Rate limiter kiểm tra: không quá 20 tin/phút
...
def check_rate_limit() -> bool:
    ...
    if len(window) >= settings.rate_limit_per_minute:
        raise RateLimitExceeded("Bạn đã gửi quá 20 tin nhắn/phút...")
...
- Fixed window: reset đúng giờ → user có thể gửi 20 tin ở phút X:59 và 20 tin ở X+1:00 → 40 trong 2 giây
- Sliding window: tại mọi thời điểm, trong 60s gần nhất không quá 20 tin → an toàn hơn
...
Railway tự tạo URL dạng `https://xanhsm-bot-production.up.railway.app`
...
### Test 5: Rate limit
Gửi hơn 20 tin nhắn trong 1 phút → bot hiển thị:
> ⚠️ Bạn đã gửi quá 20 tin nhắn/phút. Vui lòng chờ 60 giây rồi thử lại.
