...
| Hàn Quang Hiếu | Lên ý tưởng và viết hoàn chỉnh toàn bộ spec (AI Product Canvas, User Stories 4 paths, Eval metrics, Failure modes, ROI, Mini AI spec); đề xuất insight foreigners là nhóm user chính bị underserved; xây dựng tool crawl FAQ chính thức Xanh SM; crawl dữ liệu giá cước theo từng loại xe và thành phố; crawl dữ liệu thông tin dịch vụ Xanh SM; xây dựng tool thu thập dữ liệu từ Facebook Group | `spec-final.md` (toàn bộ phần 1–6); `crawlFAQ.py`; `xanhsm_faqs.json`; `Dataset/pricedata.json`; `Dataset/price-bike.json`; `Dataset/xanhsm_services.json`; `Dataset/cities_type.json` |
